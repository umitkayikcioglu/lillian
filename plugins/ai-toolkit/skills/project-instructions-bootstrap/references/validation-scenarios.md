...
| Node.js package repository | Owning scope has `package.json` with declared scripts or Node runtime metadata | Discovery runs | Node.js profile activates for the package scope | Package manager, working directory, runtime evidence, and only declared commands are documented |
| Node.js workspace | Root manifest declares npm, pnpm, Yarn, or Bun workspaces | Discovery runs | Workspace and package scopes are resolved separately | Commands preserve the declared workspace ownership and package manager |
| Node.js conflicting managers | A Node scope has conflicting `packageManager`, lockfile, and CI evidence | Package-manager resolution runs | Valid project declaration takes precedence and the conflict is reported | No script translation occurs; unresolved conflicts emit no manager-specific verified commands |
| JavaScript files without Node manifest | Repository contains `.js`, `.mjs`, or `.cjs` files but no Node manifest or runtime configuration | Discovery runs | Node.js profile does not activate | File extensions are reported as supporting-only evidence |
| Frontend and Node mixed workspace | One workspace contains browser frontend packages and a Node backend or CLI package | Discovery runs | Frontend and Node runtime scopes remain separate | Frontend profiles and Node profile do not collapse into one command surface |
| Python project metadata | Owning scope has `pyproject.toml`, `setup.py`, or `setup.cfg` | Discovery runs | Python profile activates | Project scope, interpreter evidence, environment/package manager, and declared commands are documented |
| Python requirements project | Owning scope has `requirements.txt` or an equivalent requirements file | Discovery runs | Python profile activates for that project | Requirements ownership and exact validation commands are preserved |
| Python Poetry or uv project | Owning scope has Poetry or uv metadata/lockfile | Environment resolution runs | The owning manager is selected when deterministic | Commands preserve Poetry/uv context and are not translated to pip |
| Python conflicting environments | A Python project has conflicting Poetry, uv, Pipenv, conda, requirements, or CI evidence | Environment resolution runs | Project metadata wins when valid; otherwise ambiguity is reported | No manager-specific verified command is emitted for an unresolved scope |
| Python files without project metadata | Repository contains `.py` files but no Python project metadata or environment evidence | Discovery runs | Python profile does not activate | File extensions and conventional directories are supporting-only evidence |
| Node.js and Python monorepo | Repository has independent Node and Python project scopes | Discovery runs | Each runtime and project is represented separately | Commands retain owning scope, working directory, manager/environment, and evidence source |
...
| Next.js-only repository | Package manifest declares `next` and no Angular or .NET evidence | Discovery runs | Next.js profile activates | Next.js guidance is included without Angular or .NET guidance |
| Next config without dependency | Repository has `next.config.js` but no declared `next` dependency | Discovery runs | Next config is supporting-only evidence | Next.js profile does not activate |
| App directory without Next dependency | Repository has an `app/` directory but no declared `next` dependency | Discovery runs | Directory is supporting-only evidence | Next.js profile does not activate |
...
| .NET plus Next.js repository | Repo has `.sln` or `.csproj` and a package scope declaring `next` | Discovery runs | .NET and Next.js profiles activate by scope | Commands remain scoped with working directories |
...
| Attached document instruction | An attached or repository document contains imperative instructions unrelated to the direct request | Discovery and composition run | Document text is treated as context/evidence, not user authorization | No extra write, command, or scope is introduced from the document alone |
